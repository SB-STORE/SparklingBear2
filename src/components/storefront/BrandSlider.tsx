        ...
        >
          {brands.map((brand) => (
            <SwiperSlide key={brand.slug} style={{ width: 'auto' }}>
              ...
              >
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-card border border-border flex items-center justify-center overflow-hidden group-hover:border-primary transition-colors p-3">
                  {brand.logo_url ? (
                    <img
                      src={brand.logo_url}
                      alt={brand.name}
